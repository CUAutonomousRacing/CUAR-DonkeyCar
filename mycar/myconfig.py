...
# PCA9685_I2C_BUSNUM = 6  #None will auto detect, which is fine on the pi. But other platforms should specify the bus num.
# 
# #SSD1306_128_32
...
JOYSTICK_STEERING_SCALE = 1.0       #some people want a steering that is less sensitve. This scalar is multiplied with the steering -1 to 1. It can be negative to reverse dir.
AUTO_RECORD_ON_THROTTLE = True      #if true, we will record whenever throttle is not zero. if false, you must manually toggle recording with some other trigger. Usually circle button on joystick.
CONTROLLER_TYPE='WEB_CONTROLLER'    #(ps3|ps4|xbox|nimbus|wiiu|F710|rc3)
#CONTROLLER_TYPE='TEENSY_RC'         #(ps3|ps4|xbox|nimbus|wiiu|F710|rc3)
USE_NETWORKED_JS = False            #should we listen for remote joystick control over the network?
